...
public class Note_modifier extends javax.swing.JFrame {

    /**
     * Creates new form A_Menu
     */
    public Note_modifier() {
        initComponents();
...
private void populateComboBox3(String selectedFiliere) {
 jComboBox3.removeAllItems();
    ...
    String query = "SELECT CONCAT(st.first_name, ' ', st.last_name) AS student_name " +
                   "FROM students AS st " +
                   "JOIN inscription AS ins ON st.student_id = ins.id_etudiant " +
                   "JOIN filiere AS f ON f.id = ins.id_filiere " +
                   "JOIN module AS m ON m.id_filiere = f.id " +
    ...
    private void addNote(double noteValue) {
    ...
    String query = "INSERT INTO school_management.notes (id_etudiant, id_module, note) VALUES (?, ?, ?)";
...
private void updateNote(int noteId, double noteValue) {
    ...
    if (noteText.isEmpty()) {
        JOptionPane.showMessageDialog(null, "Please enter a note.", "Error", JOptionPane.ERROR_MESSAGE);
        return;
    }
    ...
    try {
        ...
        if (selectedRow == -1) {
            // No row is selected, so add a new note
            addNote(noteValue);
        } else {
            // A row is selected, so update the existing note
            int noteId = (int) jTable1.getValueAt(selectedRow, 0); // Assuming note ID is in the first column
            updateNote(noteId, noteValue);
        }
    } catch (NumberFormatException e) {
        JOptionPane.showMessageDialog(null, "Please enter a valid number for the note.", "Error", JOptionPane.ERROR_MESSAGE);
    }
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(Note_modifier.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new Note_modifier().setVisible(true);
            }
        });
    }
